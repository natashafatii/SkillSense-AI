...
class _HrRegisterScreenState extends State<HrRegisterScreen> {
  ...
  Future<void> _register({bool isWeb = false}) async {
    if (!isWeb && !(_formKey.currentState?.validate() ?? false)) return;
      ...
      if (isWeb) {
        ...
        if (parts.isNotEmpty) {
          ...
          if (parts.length > 1) {
            ...
            lastName = parts.first; // Default to first name if single word entered
          }
        }
  ...
  InputDecoration _decor(
    ...
    return InputDecoration(
      ...
      labelStyle: GoogleFonts.publicSans(
        fontSize: 14,
        color: focused ? _blue : const Color(0xFF9E9E9E),
    ...
    child: Row(
      children: [
        ...
        Container(
          ...
          child: const Icon(Icons.more_horiz_rounded, size: 12, color: _blue),
        ),
        Expanded(
          child: Container(
            height: 1,
  ...
  Widget _icon(IconData data, {bool focused = false}) =>
      Icon(data, size: 20, color: focused ? _blue : const Color(0xFFBDBDBD));
